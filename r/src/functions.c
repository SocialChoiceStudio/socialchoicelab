...
/* r_scs_level_set_polygon_2d — level_set_2d → level_set_to_polygon in one call.
 * Arguments: ideal_x, ideal_y, utility_level, loss_cfg, dist_cfg, n_samples.
 * Returns an n×2 column-major matrix (x, y columns). */
SEXP r_scs_level_set_polygon_2d(SEXP ideal_x, SEXP ideal_y, SEXP utility_level,
                                 SEXP loss_cfg, SEXP dist_cfg,
                                 SEXP n_samples_s) {
    SCS_LossConfig lc = build_loss_config(loss_cfg);
    SCS_DistanceConfig dc = build_dist_config(dist_cfg);
    ...
    char err[SCS_ERR_BUF_SIZE] = {0};
    int out_n;
    scs_check(scs_level_set_polygon_2d(asReal(ideal_x), asReal(ideal_y),
                                      asReal(utility_level), &lc, &dc, ns,
                                      NULL, 0, &out_n, err, SCS_ERR_BUF_SIZE),
              err);
    double *tmp = (double *)R_alloc((size_t)out_n * 2, sizeof(double));
    scs_check(scs_level_set_polygon_2d(asReal(ideal_x), asReal(ideal_y),
                                      asReal(utility_level), &lc, &dc, ns,
                                      tmp, out_n, &out_n, err, SCS_ERR_BUF_SIZE),
              err);
    SEXP mat = PROTECT(allocMatrix(REALSXP, out_n, 2));
    double *dst = REAL(mat);
    for (int i = 0; i < out_n; i++) {
