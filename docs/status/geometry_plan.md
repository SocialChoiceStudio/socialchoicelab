...
| Step | Task | Deliverable | Status |
|------|------|-------------|--------|
| **F1** | **Winset set operations** | `include/socialchoicelab/geometry/winset_ops.h`: `winset_union(a, b) → Polygon2E`, `winset_intersection(a, b) → Polygon2E`, `winset_difference(a, b) → Polygon2E`, `winset_symmetric_difference(a, b) → Polygon2E` using CGAL exact polygon boolean operations. Also `winset_contains(polygon, point) → bool` and `winset_is_empty(polygon) → bool`. Tests: union of two overlapping winsets; intersection that produces empty result; containment checks. | ⬜ |
| **F2** | **Core detection** | `include/socialchoicelab/geometry/core.h`: `core_2d(voter_ideals, dist_cfg, k = n/2+1) → std::optional<Point2d>` — returns the Condorcet winner if one exists (winset is empty), otherwise `std::nullopt`. Also `has_condorcet_winner(alternatives, voter_ideals, dist_cfg, k) → bool` for finite alternative sets. Tests: symmetric 3-voter case (no Condorcet winner); asymmetric case where one alternative beats all others; verify result matches empty winset detection. | ⬜ |
| **F3** | **Copeland winner / strong point** | `copeland_winner(alternatives, voter_ideals, dist_cfg, k = n/2+1) → Point2d` and `copeland_scores(alternatives, voter_ideals, dist_cfg, k) → std::vector<int>` — score for each alternative = (number beaten) − (number that beat it). Computed from the pairwise matrix (B2). Also called the **strong point** in the spatial literature (the alternative with smallest win-set area). Tests: Condorcet cycle (all equal scores); one dominant alternative; verify strong point lies within the uncovered set; tie-breaking documented. | ⬜ |
| **F4** | **Veto players** | `winset_with_veto_2d(status_quo, voter_ideals, dist_cfg, veto_player_ideals, k = n/2+1) → Polygon2E` — computes the standard winset then intersects with each veto player's preferred region (disk centred at their ideal point with radius = their distance to the SQ). Tests: single veto player at SQ (empty winset); veto player far from SQ (no constraint); two veto players with conflicting preferences. | ⬜ |
| **F5** | **Weighted voting** | Extend `majority_prefers`, `winset_2d`, and derived functions to accept `std::vector<double> vote_weights` (default: all 1.0). The majority test becomes: sum of weights on x's side ≥ `k`% of total weight. `k` remains the fractional threshold (e.g. 0.5 for simple majority, 2/3 for supermajority). Tests: uniform weights produce same result as unweighted; one voter with double weight changes outcome; weight threshold at exactly 50% (tie). | ⬜ |

### Phase G — Heart

| Step | Task | Deliverable | Status |
|------|------|-------------|--------|
| **G1** | **Heart over a finite alternative set** | `include/socialchoicelab/geometry/heart.h`: `heart(alternatives, voter_ideals, dist_cfg, k = n/2+1) → std::vector<Point2d>` — iterative fixed-point algorithm: start with the full uncovered set; repeatedly remove any alternative x for which every alternative that beats x has something in the current set that beats it back; iterate until stable. Requires Phase B (majority) and Phase D (uncovered set). Tests: Condorcet winner case (Heart = {winner}); known 3-alternative Condorcet cycle (Heart = all three, same as uncovered set); asymmetric voter configuration where Heart is strictly smaller than uncovered set. | ⬜ |
| **G2** | **Heart boundary (continuous approximation)** | `heart_boundary_2d(voter_ideals, dist_cfg, grid_resolution, k = n/2+1) → Polygon2E` — evaluates the Heart fixed-point over a grid of candidate points in continuous policy space, then computes the convex hull of surviving points. Document that this is an approximation; exact continuous Heart computation is a research-level problem. Tests: verify Heart region is contained within the uncovered set boundary (D3); verify Heart center is near the Yolk center (C2); region shrinks as voter dispersion decreases. | ⬜ |
